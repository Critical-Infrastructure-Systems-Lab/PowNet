...
thermal_units = pd.read_csv(
    "./originals/thermal_2016.csv", header=0, usecols=["name", "node"]
)
...
DATES = pd.read_csv(
    "demand_export.csv", header=0, usecols=["year", "month", "day", "hour"]
)
...
rnw_prices = pd.concat(
    [solar_prices, wind_prices, hydro_prices, hydro_import_prices], axis=1
...
thermal_price_map = {
    row[1]["fuel_type"]: row[1]["price"] for row in thermal_price_map.iterrows()
}

thermal_units = thermal_units = pd.read_csv(
    "./originals/thermal_2016.csv", header=0, usecols=["name", "typ"]
)
...
# Thermal units
thermal_units = pd.read_csv(
    "thermal_unit", header=0, usecols=["name", "fuel_type", "unit_type"]
)
...
fuel_map = pd.DataFrame(
    {"name": generators, "unit_type": unit_type, "fuel_type": fuel_type}
)
