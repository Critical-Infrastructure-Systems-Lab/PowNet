...
system_input = SystemInput(T=T, model_name=MODEL_NAME, formulation="kirchhoff")

simulator = Simulator(system_input=system_input, write_model=True)
...
if WRITE_DEC:
    ...
    subp_thermal_map = {x: (idx + 1) for idx, x in enumerate(thermal_units)}

    subp_rnw_map = {
        x: (idx + 1 + len(subp_thermal_map)) for idx, x in enumerate(rnw_units)
    }

    subp_import_map = {
        x: (idx + 1 + len(subp_thermal_map) + len(subp_rnw_map))
        for idx, x in enumerate(nodes_import)
    }
