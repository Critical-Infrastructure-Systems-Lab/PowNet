...
def solve_release_from_storage(
    ...
    release_vars = model.addVars(
        range(start_day, end_day), lb=0, ub=max_release, name="release"
    )
    ...
    model.addConstrs(
        (release_vars[day] <= max_release for day in range(start_day, end_day)),
        name="c_max_release",
    )
    # Spill
    model.addConstrs(
    ...
    if model.status == gp.GRB.INFEASIBLE:
        # Export IIS file if the problem is infeasible
        model.computeIIS()
        model.write(f"release_from_storage.ilp")
