...
class InputProcessor:
    def __init__(
        ...
        self.transmission_file = os.path.join(
            self.model_folder, 'transmission.csv')
    ...
    def get_transmission_susceptance(self) -> None:
        ...
        self.transmission_data['pownet_reactance_pu'] =\
            source_sink['reactance_per_km'] * \
            self.transmission_data['distance']

        self.transmission_data['pownet_susceptance'] = self.transmission_data.apply(
            lambda x: int(x['source_kv'] * x['sink_kv'] /
                          x['pownet_reactance_pu']),
            axis=1)
    ...
    def create_cycle_map(self, to_write: bool = True) -> None:
        ...
        cycle_map = {f'cycle_{idx+1}': cycle for idx,
                     cycle in enumerate(cycles)}
    ...
    def create_derate_factors(
        ...
        thermal_units = pd.read_csv(os.path.join(
            model_dir, 'unit_param.csv'))['name'].values
    ...
    def create_derated_max_capacities(
        ...
        '''
        derate_path = os.path.join(
            self.model_folder, 'pownet_derate_factor.csv')
        ...
        for thermal_unit in max_cap.keys():
            derated_max_cap[thermal_unit] = derate_df[thermal_unit] * \
                max_cap[thermal_unit]
    ...
    def create_fuelprice(self) -> None:
        ...
        fuelprice = fuel_map[['name', 'fuel_price_optional']].set_index(
            'name').T
        fuelprice.index = [0]
...
if __name__ == '__main__':
    ip = InputProcessor(2016, 'dummy_hydro', 50)
